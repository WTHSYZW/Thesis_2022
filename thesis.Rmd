...
## run r5r (install Java SE Development Kit 11 from Oracle)
...
ggplot(weibo_predict) +
  ...
  geom_density(aes(x = noisy_pred_mean))
...
ggplot(weibo_predict_sf) +
  geom_sf(aes(color = weibo_predict_sf$noisy_pred_1), size = 0.1) +
  scale_color_viridis_c() +
  theme_map()
...
```
```{r}
a = weibo_predict_sf$pred
b = weibo_predict_sf$noisy_pred_1
c = weibo_clip$positive_p
...
# weibo_predict2 <- weibo_clip
# 
# weibo_predict2$restaurant_access_15 <- weibo_clip$pl_restaurant_access_15
# 
# weibo_predict2$positive_pred <- predict(Timeslot8_model, weibo_predict2)
# 
# weibo_predict2_sf = weibo_predict2 %>%
#   mutate(geom = gsub(geometry,pattern="(\\))|(\\()|c",replacement = ""))%>%
#   tidyr::separate(geom,into=c("lat","lon"),sep=",")%>%
#   st_as_sf(.,coords=c("lat","lon"),crs= 'WGS84')
# 
# ggplot(weibo_predict2_sf) +
#   geom_sf(aes(color = weibo_predict2_sf$positive_pred), size = 0.1) +
#   scale_color_viridis_c() +
#   theme_map()
# 
...
#                  bins = 30)
# 
# weibo_predict2_hist <- ggplot(weibo_predict2_sf) +
#   geom_histogram(aes(x = positive_pred),
#                  bins = 30)
# 
# weibo_existing_hist
# 
# weibo_predict2_hist
# 
# grid.arrange(weibo_existing_hist, weibo_predict2_hist,
#              ncol = 2)
# 
# ggplot(weibo_clip) +
#      geom_point(aes(x = restaurant_access_15, y= positive_p))
# ggplot(weibo_predict2) +
#      geom_point(aes(x = restaurant_access_15, y= positive_pred))
# ```
...
```
```{r, message=FALSE, warning=FALSE, results='hide'}
weibo_predict3 <- weibo_clip

weibo_predict3$restaurant_access_15 <- weibo_clip$pr_restaurant_access_15

weibo_predict3$positive_pred <- predict(Timeslot8_model, weibo_predict3)

weibo_predict3_sf = weibo_predict3 %>%
  mutate(geom = gsub(geometry,pattern="(\\))|(\\()|c",replacement = ""))%>%
  tidyr::separate(geom,into=c("lat","lon"),sep=",")%>%
  st_as_sf(.,coords=c("lat","lon"),crs= 'WGS84')

ggplot(weibo_predict3_sf) +
  geom_sf(aes(color = weibo_predict3_sf$positive_pred), size = 0.1) +
  scale_color_viridis_c() +
  theme_map()
...
weibo_predict3_hist <- ggplot(weibo_predict3) +
  geom_histogram(aes(x = positive_pred),
                 bins = 30)
...
grid.arrange(weibo_existing_hist, weibo_predict3_hist,
             ncol = 2)

ggplot(weibo_clip) +
     geom_point(aes(x = restaurant_access_15, y= positive_p))
ggplot(weibo_predict3) +
     geom_point(aes(x = restaurant_access_15, y= positive_pred))
```
```{r}
a = weibo_predict3_sf$positive_pred
B = weibo_clip$positive_p
...
ggplot() +
  geom_sf(data = boundary, color = "black") +
  geom_sf(data = school, color = 'red', size = 1) +
  geom_sf(data = doubleschool_point, color = 'green', size = 1) +
  theme_map()

ggplot() +
  geom_sf(data = boundary, color = "black") +
  geom_sf(data = school, color = 'red') +
  theme_map()

ggplot() +
  geom_sf(data = boundary, color = "black") +
  geom_sf(data = doubleschool, color = 'red') +
  theme_map()

ggplot() +
  geom_sf(data = boundary, color = "black") +
  geom_sf(data = newschool, color = 'red') +
  geom_sf(data = school, color = 'green')
  theme_map()
```
...
weibo_predict_scenario2$school_access_25 <- weibo_clip$doubleschool_access_25

weibo_predict_scenario2$positive_p <- predict(Timeslot8_model, weibo_predict_scenario2)

weibo_predict_scenario2_sf = weibo_predict_scenario2 %>%
  mutate(geom = gsub(geometry,pattern="(\\))|(\\()|c",replacement = ""))%>%
  tidyr::separate(geom,into=c("lat","lon"),sep=",")%>%
  st_as_sf(.,coords=c("lat","lon"),crs= 'WGS84')

ggplot(weibo_predict_scenario2_sf) +
  geom_sf(aes(color = weibo_predict_scenario2_sf$positive_p), size = 0.1) +
  scale_color_viridis_c() +
  theme_map()
```
```{r}
a = weibo_predict_sf$pred
b = weibo_predict_scenario2_sf$positive_p
c = weibo_predict_sf$positive_p
...
newgreen <- rbindx(transgreen, greenspace_points)

newgreen_point <- st_as_sf(x = newgreen, 
                        crs = "WGS84")
ggplot() +
  geom_sf(data = boundary, color = "black") +
  geom_sf(data = greenspace_points, color = 'red', size = 1) +
  geom_sf(data = newgreen_point, color = 'green', size = 1) +
  theme_map()
...
ggplot() +
  geom_sf(data = boundary, color = "black") +
  geom_sf(data = newgreen_point, color = 'red') +
  theme_map()
...
weibo_predict_scenario3$greenspace_access_25 <- weibo_clip$newgreen_access_25
weibo_predict_scenario3$school_access_25 <- 0

weibo_predict_scenario3$positive_p <- predict(Timeslot8_model, weibo_predict_scenario3)

weibo_predict_scenario3_sf = weibo_predict_scenario3 %>%
  mutate(geom = gsub(geometry,pattern="(\\))|(\\()|c",replacement = ""))%>%
  tidyr::separate(geom,into=c("lat","lon"),sep=",")%>%
  st_as_sf(.,coords=c("lat","lon"),crs= 'WGS84')

ggplot(weibo_predict_scenario3_sf) +
  geom_sf(aes(color = weibo_predict_scenario3_sf$positive_p), size = 0.1) +
  scale_color_viridis_c() +
  theme_map()
```
```{r}
a = weibo_predict_sf$pred
b = weibo_predict_scenario3_sf$positive_p
c = weibo_clip$positive_p
...
```{r}
JX_palette <- colorNumeric(c("cornsilk", "darkgreen"), weibo_clip$positive_p)

# ggplot(weibo_clip_sf) +
#   geom_sf(aes(color = weibo_clip_sf$positive_p), size = 0.1) +
#   scale_color_viridis_c() +
#   theme_map()

content <- paste(sep = "<br/>",
  "Possible to be Positive: ", weibo_clip$positive_p,
  "Proximity to Restaurant: ", weibo_clip$restaurant_access_15,
  "Proximity to School: ", weibo_clip$school_access_25,
  "Proximity to Hospital: ", weibo_clip$hospital_access_25,
  "Proximity to Busstop: ", weibo_clip$busstop_access_70,
  "Proximity to Greenspace: ", weibo_clip$greenspace_access_25,
  "Proximity to Leisure: ", weibo_clip$leis_access_20,
  "Proximity to Smallbusiness: ", weibo_clip$public_access_15,
  "Proximity to Shopping: ", weibo_clip$shopping_access_25
)


jx_map <- leaflet(weibo_clip_sf) %>% 
    setView(lng = 120.8090, lat = 30.68481, zoom = 13)%>%
  addProviderTiles("Stamen.TonerLite")
    
jx_map

jx_ex <- jx_map%>%
  addCircleMarkers(stroke = FALSE, color = ~JX_palette(positive_p), fillOpacity = 0.7, radius = 10,
  lng = weibo_clip$lon,
  lat = weibo_clip$lat,
  popup = content,
  options = popupOptions(closeButton = FALSE))%>%
  addCircleMarkers(data = restaurant, stroke = FALSE, color = "pink", fillOpacity = 0.5, radius = 3, group = "restaurant")%>%
  addCircleMarkers(data = school, stroke = FALSE, color = "black", fillOpacity = 0.5, radius = 3, group = "school")%>%
  addCircleMarkers(data = hospital, stroke = FALSE, color = "yellow", fillOpacity = 0.5, radius = 3, group = "hospital")%>%
  addCircleMarkers(data = busstop, stroke = FALSE, color = "orange", fillOpacity = 0.5, radius = 3, group = "busstop")%>%
  addCircleMarkers(data = greenspace_points, stroke = FALSE, color = "tin", fillOpacity = 0.5, radius = 3, group = "greenspace")%>%
  addCircleMarkers(data = leis, stroke = FALSE, color = "red", fillOpacity = 0.5, radius = 3, group = "leis")%>%
  addCircleMarkers(data = public, stroke = FALSE, color = "blue", fillOpacity = 0.5, radius = 3, group = "public")%>%
  addCircleMarkers(data = shopping, stroke = FALSE, color = "purple", fillOpacity = 0.5, radius = 3, group = "shopping")%>%
  addLayersControl(
    overlayGroups = c("restaurant","school","hospital","busstop","greenspace","leis","public","shopping"),
    options = layersControlOptions(collapsed = FALSE))

saveWidget(jx_ex, file="jx_ex.html")
```
...
saveWidget(jx_ex, file="jx_ex2.html")
```

## scenario 1 planned condition interactive map

```{r}
JX_palette1 <- colorNumeric(c("cornsilk", "darkgreen"), weibo_predict3_sf$positive_pred)
...
content1 <- paste(sep = "<br/>",
  "Possible to be Positive: ", weibo_predict3_sf$positive_pred,
  "Proximity to Restaurant: ", weibo_predict3_sf$restaurant_access_15,
  "Proximity to School: ", weibo_predict3_sf$school_access_25,
  "Proximity to Hospital: ", weibo_predict3_sf$hospital_access_25,
  "Proximity to Busstop: ", weibo_predict3_sf$busstop_access_70,
  "Proximity to Greenspace: ", weibo_predict3_sf$greenspace_access_25,
  "Proximity to Leisure: ", weibo_predict3_sf$leis_access_20,
  "Proximity to Smallbusiness: ", weibo_predict3_sf$public_access_15,
  "Proximity to Shopping: ", weibo_predict3_sf$shopping_access_25
)


jx_map_scenario1 <- leaflet(weibo_predict3_sf) %>% 
    setView(lng = 120.8090, lat = 30.68481, zoom = 13)%>%
  addProviderTiles("Stamen.TonerLite")
    
jx_map_scenario1

jx_map_scenario1 <- jx_map_scenario1%>%
  addCircleMarkers(data = weibo_predict3_sf, stroke = FALSE, color = ~JX_palette1(positive_pred), fillOpacity = 0.7, radius = 10,
  popup = content1,
  options = popupOptions(closeButton = FALSE))%>%
  addCircleMarkers(data = predict_res_point, stroke = FALSE, color = "brown", fillOpacity = 0.5, radius = 3, group = "Planned restaurant")%>%
  addCircleMarkers(data = restaurant, stroke = FALSE, color = "pink", fillOpacity = 0.5, radius = 3, group = "Original restaurant")%>%
  addCircleMarkers(data = school, stroke = FALSE, color = "black", fillOpacity = 0.5, radius = 3, group = "school")%>%
  addCircleMarkers(data = hospital, stroke = FALSE, color = "yellow", fillOpacity = 0.5, radius = 3, group = "hospital")%>%
  addCircleMarkers(data = busstop, stroke = FALSE, color = "orange", fillOpacity = 0.5, radius = 3, group = "busstop")%>%
  addCircleMarkers(data = greenspace_points, stroke = FALSE, color = "tin", fillOpacity = 0.5, radius = 3, group = "greenspace")%>%
  ...
  addCircleMarkers(data = shopping, stroke = FALSE, color = "purple", fillOpacity = 0.5, radius = 3, group = "shopping")%>%
  addLayersControl(
    overlayGroups = c("Planned restaurant","Original restaurant","school","hospital","busstop","greenspace","leis","public","shopping"),
    options = layersControlOptions(collapsed = FALSE))

saveWidget(jx_map_scenario1, file="jx_map_scenario1.html")
```

## scenario 2 school interactive map

```{r}
JX_palette2 <- colorNumeric(c("cornsilk", "darkgreen"), weibo_predict_scenario2_sf$positive_p)

content2 <- paste(sep = "<br/>",
  "Possible to be Positive: ", weibo_predict_scenario2_sf$positive_p,
  "Proximity to Restaurant: ", weibo_predict_scenario2_sf$restaurant_access_15,
  "Proximity to School: ", weibo_predict_scenario2_sf$school_access_25,
  "Proximity to Hospital: ", weibo_predict_scenario2_sf$hospital_access_25,
  "Proximity to Busstop: ", weibo_predict_scenario2_sf$busstop_access_70,
  "Proximity to Greenspace: ", weibo_predict_scenario2_sf$greenspace_access_25,
  "Proximity to Leisure: ", weibo_predict_scenario2_sf$leis_access_20,
  "Proximity to Smallbusiness: ", weibo_predict_scenario2_sf$public_access_15,
  "Proximity to Shopping: ", weibo_predict_scenario2_sf$shopping_access_25
)


jx_map_scenario2 <- leaflet(weibo_predict_scenario2_sf) %>% 
    setView(lng = 120.8090, lat = 30.68481, zoom = 13)%>%
  addProviderTiles("Stamen.TonerLite")

jx_map_scenario2 <- jx_map_scenario2%>%
  addCircleMarkers(data = weibo_predict_scenario2_sf, stroke = FALSE, color = ~JX_palette2(positive_p), fillOpacity = 0.7, radius = 10,
  popup = content2,
  options = popupOptions(closeButton = FALSE))%>%
  addCircleMarkers(data = restaurant, stroke = FALSE, color = "pink", fillOpacity = 0.5, radius = 3, group = "restaurant")%>%
  addCircleMarkers(data = doubleschool_point, stroke = FALSE, color = "brown", fillOpacity = 0.5, radius = 3, group = "Double school")%>%
  addCircleMarkers(data = school, stroke = FALSE, color = "grey", fillOpacity = 0.5, radius = 3, group = "Original school")%>%
  addCircleMarkers(data = hospital, stroke = FALSE, color = "yellow", fillOpacity = 0.5, radius = 3, group = "hospital")%>%
  addCircleMarkers(data = busstop, stroke = FALSE, color = "orange", fillOpacity = 0.5, radius = 3, group = "busstop")%>%
  addCircleMarkers(data = greenspace_points, stroke = FALSE, color = "tin", fillOpacity = 0.5, radius = 3, group = "greenspace")%>%
  addCircleMarkers(data = leis, stroke = FALSE, color = "red", fillOpacity = 0.5, radius = 3, group = "leis")%>%
  ...
  addCircleMarkers(data = shopping, stroke = FALSE, color = "purple", fillOpacity = 0.5, radius = 3, group = "shopping")%>%
  addLayersControl(
    overlayGroups = c("restaurant","Double school","Original school","hospital","busstop","greenspace","leis","public","shopping"),
    options = layersControlOptions(collapsed = FALSE))

saveWidget(jx_map_scenario2, file="jx_map_scenario2.html")
```

## scenario 3 school into greenspace interactive map

```{r}
JX_palette3 <- colorNumeric(c("cornsilk", "darkgreen"), weibo_predict_scenario3_sf$positive_p)

content3 <- paste(sep = "<br/>",
  "Possible to be Positive: ", weibo_predict_scenario3_sf$positive_p,
  "Proximity to Restaurant: ", weibo_predict_scenario3_sf$restaurant_access_15,
  "Proximity to School: ", weibo_predict_scenario3_sf$school_access_25,
  "Proximity to Hospital: ", weibo_predict_scenario3_sf$hospital_access_25,
  "Proximity to Busstop: ", weibo_predict_scenario3_sf$busstop_access_70,
  "Proximity to Greenspace: ", weibo_predict_scenario3_sf$greenspace_access_25,
  "Proximity to Leisure: ", weibo_predict_scenario3_sf$leis_access_20,
  "Proximity to Smallbusiness: ", weibo_predict_scenario3_sf$public_access_15,
  "Proximity to Shopping: ", weibo_predict_scenario3_sf$shopping_access_25
)


jx_map_scenario3 <- leaflet(weibo_predict_scenario3_sf) %>% 
    setView(lng = 120.8090, lat = 30.68481, zoom = 13)%>%
  addProviderTiles("Stamen.TonerLite")

jx_map_scenario3 <- jx_map_scenario3%>%
  addCircleMarkers(data = weibo_predict_scenario3_sf, stroke = FALSE, color = ~JX_palette3(positive_p), fillOpacity = 0.7, radius = 10,
  popup = content3,
  options = popupOptions(closeButton = FALSE))%>%
  addCircleMarkers(data = restaurant, stroke = FALSE, color = "pink", fillOpacity = 0.5, radius = 3, group = "restaurant")%>%
  addCircleMarkers(data = school, stroke = FALSE, color = "grey", fillOpacity = 0.5, radius = 3, group = "Original school")%>%
  addCircleMarkers(data = hospital, stroke = FALSE, color = "yellow", fillOpacity = 0.5, radius = 3, group = "hospital")%>%
  addCircleMarkers(data = busstop, stroke = FALSE, color = "orange", fillOpacity = 0.5, radius = 3, group = "busstop")%>%
  addCircleMarkers(data = newgreen_point, stroke = FALSE, color = "brown", fillOpacity = 0.5, radius = 3, group = "New greenspace")%>%
  addCircleMarkers(data = greenspace_points, stroke = FALSE, color = "tin", fillOpacity = 0.5, radius = 3, group = "Original greenspace")%>%
  addCircleMarkers(data = leis, stroke = FALSE, color = "red", fillOpacity = 0.5, radius = 3, group = "leis")%>%
  addCircleMarkers(data = public, stroke = FALSE, color = "blue", fillOpacity = 0.5, radius = 3, group = "public")%>%
  addCircleMarkers(data = shopping, stroke = FALSE, color = "purple", fillOpacity = 0.5, radius = 3, group = "shopping")%>%
  addLayersControl(
    overlayGroups = c("restaurant","Original school","hospital","busstop","New greenspace","Original greenspace","leis","public","shopping"),
    options = layersControlOptions(collapsed = FALSE))

saveWidget(jx_map_scenario3, file="jx_map_scenario3.html")
```
